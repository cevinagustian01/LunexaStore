...
// 1. Extract head and navbar from cms.html up to the Navigation Tabs
...
// We also need the login logic which is in cms.html
// But let's write the specific bot-cms body
const botBody = \
        <div class="d-flex justify-content-between align-items-end mb-4">
            <div>
                <h2 class="fw-bold" style="color: var(--primary);">Bot Dashboard</h2>
                <p class="text-muted mb-0">Kelola pesanan dan produk Telegram Bot (Toko 2).</p>
            </div>
        ...
        <div id="login-screen" class="glass-card text-center p-5 mb-4" style="max-width: 500px; margin: 0 auto;">
            <div class="mb-4">
        ...
        <div id="dashboard-content" class="d-none">
            ...
            <ul class="nav nav-pills mb-4 gap-2">
                <li class="nav-item">
                    <button class="nav-link active btn-brand" id="btn-tab-orders" onclick="switchView('orders')" style="border-radius:20px;">Riwayat Transaksi Bot</button>
                </li>
                <li class="nav-item">
                    <button class="nav-link btn-outline-brand" id="btn-tab-products" onclick="switchView('products')" style="border-radius:20px;">Produk Toko 2</button>
                </li>
            </ul>
            ...
            <div id="view-products" class="glass-card d-none">
                <div class="d-flex justify-content-between align-items-center mb-3">
                    <h5 class="fw-bold mb-0"><i class="fas fa-box-open me-2"></i>Produk Toko 2 (Bot)</h5>
    ...
    <script>
        ...
        document.getElementById('tab-cms').className = 'btn btn-outline-brand';

        let CMS_KEY = localStorage.getItem('adm_key') || '';
        ...
        async function login() {
            ...
            try {
                ...
                document.getElementById('login-msg').textContent = 'Server Bot Error. Pastikan bot menyala & port dibuka.';
            }
        }
        ...
        async function init() {
            ...
            try {
                ...
                document.getElementById('login-msg').textContent = 'Session expired / Bot Offline';
                localStorage.removeItem('adm_key');
            }
        }

        function switchView(tab) {
            document.getElementById('btn-tab-orders').className = 'nav-link btn-outline-brand';
            document.getElementById('btn-tab-products').className = 'nav-link btn-outline-brand';
            document.getElementById('view-orders').classList.add('d-none');
            document.getElementById('view-products').classList.add('d-none');
            
            document.getElementById(\tn-tab-\\).className = 'nav-link active btn-brand';
...
// Also update cms.html and cms-cookies.html to use the new Navigation Tabs
// We need to replace the old <!-- Navigation Tabs --> block
function replaceTabs(filename) {
    let html = fs.readFileSync(filename, 'utf8');
    html = html.replace(/<!-- Navigation Tabs -->[\s\S]*?<\/div>/, newTabs.trim());
    
    // Set active tab logic
    if (filename.includes('cms-cookies')) {
        html = html.replace(/id="tab-cookies" style="border-radius: 20px; text-decoration: none;"/, 'id="tab-cookies" class="btn btn-brand" style="border-radius: 20px; text-decoration: none;"');
        html = html.replace(/<a href="cms.html" class="btn btn-brand"/, '<a href="cms.html" class="btn btn-outline-brand"');
    } else {
        html = html.replace(/<a href="cms.html" class="btn btn-outline-brand"/, '<a href="cms.html" class="btn btn-brand"');
    }
