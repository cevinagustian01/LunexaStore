...
// 1. Get the top part of cms.html (up to the end of Navigation Tabs)
...
// 2. Get the actual Dashboard content from bot-cms.html (everything inside #dashboard-content and the login screen)
const loginRegex = /<div id="login-screen"[\s\S]*?<\/div>\s*<\/div>\s*<\/div>\s*<!-- Product Modal -->/;
const match = botHtml.match(loginRegex);
let botContent = match ? match[0] : '';

// Also we need to get the bottom script part from bot-cms.html
const scriptRegex = /<script src="https:\/\/cdn\.jsdelivr\.net\/npm\/bootstrap@5\.3\.0\/dist\/js\/bootstrap\.bundle\.min\.js"><\/script>[\s\S]*?<\/html>/;
const scriptMatch = botHtml.match(scriptRegex);
let botScript = scriptMatch ? scriptMatch[0] : '';

// 3. Let's fix the botScript: we removed switchView that changes active link colors. 
// Wait, the grid buttons in botContent don't have "active" state anyway. They just call switchView('orders').
// But we still need switchView logic from bot-cms.html.
...
<body>
    ...
    <div id="app">
        ...
        <div class="container-fluid" style="max-width: 1400px; padding: 0 30px;">
            ...
            ${botContent.replace('<!-- Product Modal -->', '</div> <!-- end container -->\n    <!-- Product Modal -->')}
    ${botScript}
`;

fs.writeFileSync('public/bot-cms.html', fullNewBotHtml, 'utf8');
console.log('Restored top navbar and style');
